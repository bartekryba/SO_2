...
parse_other:
    ...
    ; 0 arg operations:
    cmp     al, '='                 ; already disabled reading mode, ignore
    je      main_loop
    cmp     al, 'n'
    je      _n
    cmp     al, 'N'
    ...
    cmp     al, 0                   ; in case of null character, jump to exir
    je      exit
...
_g:
    ...
    add     rsp, rax                ; if not negative, just add it to rsp
    jmp     main_loop

_g_negative:
    neg     rax                     ; if negative, perform arithmetic negation
    sub     rsp, rax                ; and subtract it from rsp
    jmp     main_loop
...
_W_wait_for_second:                 ; wait for desired notec to connect
    mov     r11, [r13 + r9*8]       ; load wait_table value for desired notec
    sub     r11, 1
    ...
    mov     qword [rdx], 0          ; free lock that was passed from second thread
...
_W_second_wait:
    mov     r11, [r13 + r14*8]      ; wait for wait_table value for current notec to become 0
    test    r11, r11
    jnz     _W_second_wait
...
exit:
    pop     rax                     ; return value from top of the stack in rax
